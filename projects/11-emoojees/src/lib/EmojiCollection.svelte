<script lang='ts'>
  ...
  let categories_selected_arr: string[] = JSON.parse($categories_selected)
...
{#if search_query.length === 0}
  <div
    class='relative flex flex-wrap max-w-2xl mx-auto mb-10 font-medium text-white select-none w-fit place-content-evenly'
  >
            ...
            {#if emoji.name.includes(search_query) || search_query.length === 0}
              <button
                ...
                on:click={() => {
                  ...
                  addToast('Copied!', emoji_mode, `:${emoji.name}:`, 2000)
                }}
              >
                  ...
                  <button
                    ...
                    on:click={() => {
                      if (emoji_mode === 'code') {
                        ...
                        addToast('Copied!', emoji_mode, `:${slug}:`, 2000)
                      } else {
                        navigator.clipboard.writeText(emoji.emoji)
                        addToast('Copied!', emoji_mode, emoji.emoji, 2000)
                      }
                    }}
                  >
